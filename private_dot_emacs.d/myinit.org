...
#+begin_src emacs-lisp
  (use-package org-roam
    :straight t
    :init
    ;; This is necessary for the v2 database to work
    (setq org-roam-v2-ack t)
    :custom
    ;; Change this to your preferred notes folder
    ...
    :bind (("C-c n l" . org-roam-buffer-toggle)
  	 ...
  	 ("C-c n t" . org-roam-tag-add)
           ;; Dailies (Optional but recommended)
           ("C-c n j" . org-roam-dailies-capture-today))
    :config
    ;; Ensure the database syncs on startup
    (org-roam-db-autosync-mode))

  (setq org-roam-capture-templates
    '(("b" "Bibliography reference" entry
        (file "/home/tgaref/notes/org-roam-templates/literature.tmpl")
        :target (file "bibliography.org")
        :empty-lines-before 1    
        :unnarrowed t)
      ("n" "Note" plain
           (file "/home/tgaref/notes/org-roam-templates/note.tmpl")
...
#+begin_src emacs-lisp
  ...
  ;; Suppress warning about unsupported didChangeConfiguration capability
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package typst-ts-mode
    ...
    ;; Ensure Emacs can find the tree-sitter grammar
    (add-to-list 'treesit-extra-load-path (expand-file-name "tree-sitter" user-emacs-directory))
    ;; Explicitly prefer typst-ts-mode for .typ files
        ...
        (setq openwith-associations
              (list
               ...
               (list (openwith-make-extension-regexp
                     ...
                     '(file))
  	     (list (openwith-make-extension-regexp 
  		    '("doc" "docx" "odt" "ods" "xls" "xlsx" "odp" "pps" "ppt" "pptx"))
  		   "xdg-open" '(file))
               (list (openwith-make-extension-regexp
...
#+BEGIN_SRC emacs-lisp
    ...
    (use-package multi-vterm 
      ...
      :init
       ...
       (bind-key "C-/" 'multi-vterm-next) ; Control - PageUp
...
#+BEGIN_SRC emacs-lisp
  (use-package eat
    :straight (:host codeberg
               :repo "akib/emacs-eat"
               :files ("*.el" ("term" "term/*.el") "*.texi" "*.org" "terminfo"))
    :bind (("C-c t" . eat)
           ("C-c n" . my/eat-new-instance))
    :config
    ;; Note: eat-enable-mouse is a boolean variable, not a function!
    ;; It is enabled by default, so we don't need to add it to eat-mode-hook.

    (defun my/eat-new-instance ()
      "Spawn a fresh, fully functional, uniquely named instance of Eat."
      (interactive)
      ;; Rename any existing *eat* buffer first to free the namespace
      (let ((existing-buffer (get-buffer "*eat*")))
        (when existing-buffer
          (with-current-buffer existing-buffer
            (rename-buffer (generate-new-buffer-name "*eat*")))))
      ;; Spawn the new session
      (eat)
      ;; Rename the new session immediately
...
* mu4e
#+BEGIN_SRC emacs-lisp
      ;; 1. Pre-load the path so Emacs knows where to look
      (add-to-list 'load-path "/usr/share/emacs/site-lisp/mu4e/")

      (use-package mu4e
        :straight (:type built-in)
        :ensure nil
        :defer nil ; Best to not defer while debugging this error
        :init
        (require 'mu4e)
        :config
        ;; --- Global Settings ---
        ...
        ;; Don't add the 'T' (trash) flag, just move the file. 
        ;; This allows mbsync to handle the move naturally.
        (setq mu4e-trash-folder-uses-trash-flag nil)

        ;; --- Contexts ---
        ;; We use `(list ...) with make-mu4e-context inside the config 
        ;; so it only runs once mu4e is fully loaded.
        (setq mu4e-contexts
              (list
               ;; 1. GMAIL
               (make-mu4e-context
                ...
                :vars '((user-mail-address  . "tgaref@gmail.com")
               ...
               (make-mu4e-context
                :name "Infomaniak"
                :match-func (lambda (msg) (when msg (string-prefix-p "/infomaniak" (mu4e-message-field msg :maildir))))
                :vars '((user-mail-address  . "tgaref@ik.me")
                        (mu4e-sent-folder   . "/infomaniak/Sent")
                        (mu4e-trash-folder  . "/infomaniak/Trash")
                        (message-sendmail-extra-arguments . ("--account=infomaniak"))))

               ;; 3. UoC
               (make-mu4e-context
                :name "UoC"
                :match-func (lambda (msg) (when msg (string-prefix-p "/uoc" (mu4e-message-field msg :maildir))))
                :vars '((user-mail-address  . "tgaref@uoc.gr")
                        (mu4e-sent-folder   . "/uoc/Sent")
                        (mu4e-trash-folder  . "/uoc/Trash")
                        (message-sendmail-extra-arguments . ("--account=uoc"))))))

        ;; Shortcuts for jumping to inboxes (Press 'j' then 'g', 'i', or 'u')
        (setq mu4e-maildir-shortcuts
              '(("/gmail/Inbox"      . ?g)
                ("/infomaniak/Inbox" . ?i)
                ("/uoc/Inbox"        . ?u)))
        (local-set-key (kbd "C-c ") 'message-send-and-exit)
        ;; Force mu4e to show plain text if possible
        (setq mu4e-view-prefer-html nil)
        ;; If mu4e still shows HTML because the text part is too short, 
        ;; you can disable that "heuristic" entirely:
        (setq mu4e-view-html-plaintext-ratio-heuristic most-positive-fixnum)
        (set-face-attribute 'variable-pitch nil 
                        :font "Fira Sans" 
                        :height 152)
        ;;(set-face-attribute 'variable-pitch nil :height 0.85) ; 90% of the default size

        ;; 1. The "Sterile" Send Function
        (defun my-mu4e-safe-send-and-exit ()
      	"Sends the message while blinding Emacs to external PDF handlers."
      	(interactive)
      	;; This 'let' block temporarily wipes the variables that trigger external viewers
      	(let ((file-name-handler-alist nil)
                (openwith-mode nil)) 
      	  (message-send-and-exit)))

        ;; 2. The Hook to bind it to Ctrl+Enter
        (add-hook 'mu4e-compose-mode-hook
      	    (lambda ()
                    (local-set-key (kbd "<C-return>") 'my-mu4e-safe-send-and-exit)))
        
        (setq mu4e-attachment-dir "~/Downloads")
        (setq mu4e-headers-show-threads t)
        (setq mu4e-headers-omit-prefix nil)
        (setq mu4e-use-fancy-chars t)
        ;; 2. Fix the thread lines/folding background
        (set-face-attribute 'mu4e-thread-fold-face nil 
                            :background 'unspecified :inherit 'default)
        
        ;; 3. Fix "related" messages (often shown in a different color)
        (set-face-attribute 'mu4e-related-face nil 
                            :background 'unspecified :slant 'italic))
  ...
  (defvar my-signature-alist
  '(("Work"     . "Theodoulos Garefalakis\nDepartment of Mathematics\nand Applied Mathematics\nUniversity of Crete\nGR-70013 Heraklion, Greece\n\nTel. (+30) 2810-393845\nWeb: www.math.uoc.gr/~tgaref")
    ("Personal" . "")
    ("Short"    . "")
    ("None"     . ""))
  "An alist of signatures for mu4e.")

(defun my-insert-signature-choice ()
  "Prompt for a signature from my-signature-alist and insert it at point."
  (interactive)
  (let* ((choices (mapcar #'car my-signature-alist))
         (selection (completing-read "Choose signature: " choices))
         (sig (cdr (assoc selection my-signature-alist))))
    (when (and sig (not (string-empty-p sig)))
      (insert "\n-- \n" sig "\n")))) 

;; Ensure it binds to the correct map
(with-eval-after-load 'message
  (define-key message-mode-map (kbd "C-c s") #'my-insert-signature-choice))

#+END_SRC        
* agent-shell
#+BEGIN_SRC emacs-lisp
;; 1. Install the protocol handler
(straight-use-package '(acp :host github :repo "xenodium/acp.el"))

;; 2. Install the shell utility
(straight-use-package 'shell-maker)

;; 3. Install agent-shell itself
